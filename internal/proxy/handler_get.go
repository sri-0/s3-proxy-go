...
func handleGetObject(rc *RouteConfig, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, backend.ErrNotFound) {
			log.Printf("GetObject %s/%s: .meta not found", bucket, key)
			http.Error(w, "Forbidden: no security metadata for object", http.StatusForbidden)
			return
		}
		log.Printf("GetObject %s/%s: failed to fetch .meta: %v", bucket, key, err)
	...
	if err := json.Unmarshal(metaBytes, &objMeta); err != nil {
		log.Printf("GetObject %s/%s: invalid .meta JSON: %v", bucket, key, err)
	...
	if !auth.ValidateSecurityTag(objMeta.SecurityTagID, roles) {
		log.Printf("GetObject %s/%s: role mismatch, tag=%s roles=%v", bucket, key, objMeta.SecurityTagID, roles)
		http.Error(w, "Forbidden: security tag mismatch", http.StatusForbidden)
		return
	}

	objRC, objInfo, err := rc.Backend.GetObject(r.Context(), bucket, key)
	if err != nil {
		if errors.Is(err, backend.ErrNotFound) {
